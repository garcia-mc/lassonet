...
data=cox_gen(z) # 999 means infinity
...
theta=[0.7] # try to change it, final plot will only work with theta = 0.7
...
Zimportant=z[Jinford[1,:].astype(int)].transpose()

Jinford=np.vstack((Jinford,Zimportant))
gcm=Lambda0

# gompertz cumulative hazards function

# gamma=5
# lam=0.8

# H=(lam/gamma)*(np.exp(gamma*Jinford[0,:])-1)*np.exp(-np.matmul(z[Jinford[1,:].astype(int),:],theta))
...
# plt.scatter(Jinford[0,Jinford[7,:]==1],Jinford[6,Jinford[7,:]==1],s=0.5,c='red')
